...
class composite(QtWidgets.QDialog, FORM_CLASS):
    def __init__(self, bbox, date_start, date_end, cloud, user=None, password=None, parent=None):
        super(composite, self).__init__(parent)
        self.setupUi(self)
        ...
        self.pixmap = self.get_composite_preview(
            client_id=user,
            client_secret=password,
            bbox=bbox,
        ...
        self.radioButton_RGB.toggled.connect(lambda: self.update_preview_composite(bbox, date_start, date_end, cloud, user, password))
        self.radioButton_NBR.toggled.connect(lambda: self.update_preview_composite(bbox, date_start, date_end, cloud, user, password))
    ...
    def get_composite_preview(self, client_id, client_secret, bbox, date_start, date_end, cloud, width=512, height=512):
        ...
        if self.radioButton_RGB.isChecked():
            ...
            payload = {
                ...
                function evaluatePixel(samples) {
                    return [(samples.B08 - samples.B12) / (samples.B08 + samples.B12)];
                }
                """
            }
    ...
    def update_preview_composite(self, bbox, date_start, date_end, cloud, user, password):

        self.pixmap = self.get_composite_preview(user, password, bbox, date_start, date_end, cloud)
        if self.pixmap:
            self.draw_scene()
        else:
            print("Impossible to get the image.")